<div class="mx-auto max-w-sm">
    <div class="rounded-3xl border border-stone-100 bg-white p-6 shadow-sm">
        ...
        <form wire:submit="connecter" class="mt-6 space-y-4">
            ...
            <div>
                ...
                <input type="password" wire:model="password"
                       class="w-full rounded-xl border-2 border-stone-200 bg-white px-4 py-3.5 text-base text-stone-900 shadow-sm outline-none transition focus:border-[#A1763C]">
                @error('password') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
            </div>
            <label class="flex items-center gap-2 text-sm text-stone-600">
                <input type="checkbox" wire:model="remember" class="rounded border-stone-300 text-[#A1763C] focus:ring-0">
                Se souvenir de moi
            </label>
        ...
        <p class="mt-5 text-center text-sm text-stone-500">
            ...
            <a href="{{ route('shop.register') }}" wire:navigate class="font-semibold" style="color:#A1763C;">Créer un compte</a>
        </p>
    </div>
</div>
