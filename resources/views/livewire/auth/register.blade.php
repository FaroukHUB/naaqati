<div class="mx-auto max-w-sm">
    <div class="rounded-3xl border border-stone-100 bg-white p-6 shadow-sm">
        ...
        <form wire:submit="inscrire" class="mt-6 space-y-4">
            @php $cls = 'w-full rounded-xl border-2 border-stone-200 bg-white px-4 py-3.5 text-base text-stone-900 placeholder-stone-400 shadow-sm outline-none transition focus:border-[#A1763C]'; @endphp
            <div>
            ...
            <div>
                <label class="mb-1 block text-sm font-semibold text-stone-700">Mot de passe <span style="color:#A1763C;">*</span></label>
                <input type="password" wire:model="password" class="{{ $cls }}">
                @error('password') <span class="text-xs text-red-500">{{ $message }}</span> @enderror
            </div>
            <div>
                <label class="mb-1 block text-sm font-semibold text-stone-700">Confirmer le mot de passe <span style="color:#A1763C;">*</span></label>
            ...
            <button type="submit" wire:loading.attr="disabled"
                    class="w-full rounded-xl py-3 text-sm font-semibold text-white transition hover:opacity-90 disabled:opacity-50" style="background-color:#A1763C;">
                Créer mon compte
            </button>
        </form>
        ...
        <p class="mt-5 text-center text-sm text-stone-500">
            Déjà un compte ?
            <a href="{{ route('shop.login') }}" wire:navigate class="font-semibold" style="color:#A1763C;">Se connecter</a>
        </p>
    </div>
</div>
